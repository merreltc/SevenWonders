...
public class TurnHandler {
	...
	public String endCurrentPlayerTurn(Handlers handlers) {
		...
		applyCurrentPlayerEffects(true);
		applyNextPlayerEffects();
	...
	private void applyCurrentPlayerEffects(boolean sixthTurn) {
		Player current = this.turnInfo.board.getCurrentPlayer();
		HashMap<Frequency, HashSet<Effect>> wonderPile = current.getWonderPile();
		for (Frequency frequency : wonderPile.keySet()) {
			if (frequency == Frequency.SIXTHTURN && sixthTurn) {
				String play = getChosenString("playordiscard");
				if (play.equals("Play")) {
					this.turnInfo.handlers.getPlayerTurnHandler().buildStructure(current,
							current.getCurrentHand().get(0));
				} else {
					this.turnInfo.board.addToDiscardPile(current, current.getCurrentHand().get(0));
				}
			} else if (frequency == Frequency.ENDOFTURN) {
				this.turnInfo.handlers.getEffectHandler().enableAbilityFreeBuildFromDiscard(current);
			}
		}
	}

	private void applyNextPlayerEffects() {
		Player next = this.turnInfo.board.getNextPlayer();
		HashMap<Frequency, HashSet<Effect>> wonderPile = next.getWonderPile();
		for (Frequency frequency : wonderPile.keySet()) {
			if (frequency == Frequency.EVERYTURN) {
	...
	private String rotateHand(int turnsTilEnd) {
		String message;
		this.turnInfo.handlers.getRotateHandler().rotateCurrentHands(this.turnInfo.board.getPlayers(),
				this.turnInfo.currentDirection);
	...
	public Deck switchDeck(Age currentAge) {
		...
		newDeck = this.turnInfo.handlers.getSetUpDeckHandler().createDeck(nextAge,
				this.turnInfo.board.getPlayers().size());
